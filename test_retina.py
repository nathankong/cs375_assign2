...
def return_outputs(inputs, outputs, **kwargs):
    print("returnOutputs")
    metrics_dict = {}
    metrics_dict['white_noise_testcorr'] = self.pearson_correlation(logits=outputs['pred'], labels=inputs['labels'])
    return metrics_dict
...
def pearsonAggFunc(inputs, outputs, **kwargs):
        print("returnOutputs")
        metrics_dict = {}
        metrics_dict['white_noise_testcorr'] = pearson_correlation(logits=outputs['pred'], labels=inputs['labels'])
        return metrics_dict
...
default_params = {
    'save_params': {
        ...
        'dbname': 'cnn',
        'collname': stim_type,
        'exp_id': 'trainval0' + '_' + str(extraction_step),
    },
    ...
    'load_params': {
        ...
        'exp_id': 'trainval0',
        'do_restore': True,
        'query': {'step': extraction_step} \
        if extraction_step is not None else None,
        'db_restore': False,
    },
    ...
    'validation_params': {
        'test_loss': {
            ...
            'num_steps': 500,#N_TEST // MB_SIZE + 1,
...
def test_cnn():
    ...
    params['save_params']['exp_id'] = 'trainval0_1050'
    ...
    params.pop('learning_rate_params',None)
    params.pop('loss_params',None)
    params.pop('optimizer_params',None)
    params.pop('train_params',None)
